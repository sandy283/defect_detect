...
def app_function(image):
    ...
    # Run the YOLO command for defect detection (no need to use its result)
    command = ["yolo", "predict", "model=sam2.1_b.pt", "source=selected_image"]
    subprocess.run(command, text=True, capture_output=True)

    # Use the Gemini model to generate the defect detection response
